...
const addPictures = () =>{
    ...
    for(let i = 0; i < array.length; i++){
        ...
        pictures.setAttribute("loading", "lazy");
...
//    create first slide when we go
   ...
   const removeFunction = (buttons) =>{
       button.forEach(items =>{
           ...
           items.classList.remove('btn-checked');
       })
       buttons.setAttribute('disabled', 'disabled');
       buttons.classList.add('btn-checked');
   } 
...
// remove element with 0% sale
const removeElementZero = () =>{
    const tagSale1 = document.querySelectorAll('.rate-sale'); 
    const firstPrice = document.querySelectorAll('.first-price');
    tagSale1.forEach((items, index) =>{
        const content = items.innerText;
        const content2 = content.replace('-', '');
        const content3 = Number(content2.replace('%', ''));
        if(content3 === 0){
            tagSale1[index].remove();
            firstPrice[index].remove();
        }
    })
}
removeElementZero();
